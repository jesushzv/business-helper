...
describe('requireOrgAccess picks a tenant deterministically', () => {
  it('orders the owned-organizations lookup instead of taking an arbitrary row', async () => {
    ...
    // Two keys since #109, not one: `created_at` alone is not a total order.
    // `now()` is fixed for a whole transaction, so two organizations inserted
    // together share a timestamp exactly, and the ordering would still be
    // ambiguous in the case it exists to remove.
    expect(organizations?.order).toEqual([
