...
2. **Test File Target**:
   * Add unit tests to `scripts/test-runner.js` covering tax calculations, Modulo 11 check digits, data transformers, or input sanitizers.
3. **Verify Red Phase**:
   * Run `npm run test` and confirm the new unit tests fail as expected prior to implementation.
...
2. **Coverage Gate Verification**:
   * Command: `npm run test` (Must achieve `>= 85%` line/branch coverage).
