...
| **Product analytics** | Instrumentation merged (#37): seven events fire at their boundaries, each carrying `organization_id`. Inert until `POSTHOG_API_KEY` is set in Vercel — no key, no events, and a weak result stays uninterpretable. | **Yes, until the key is set** — see P0 §03 |
...
| 5 | **Wire product analytics before the first user arrives.** ⚙️ **Code landed** — the seven events from `signup_started` to `payment_confirmed` fire at their boundaries, each carrying `organization_id` ([`product-analytics.md`](product-analytics.md)). What remains is operational and takes minutes: create the PostHog project, set `POSTHOG_API_KEY` in Vercel, and build the funnel view. Until the key is set every capture is a no-op, so this item is **not** closed by the merge. | [#37](https://github.com/jesushzv/business-helper/issues/37) |
