...
- [x] **Provider Code Paths**: `lib/otpDelivery.ts` implements Twilio SMS, Twilio WhatsApp, and Meta Cloud API, selected by `OTP_DELIVERY_CHANNEL`. Fails closed outside development (PR #16; sms was retired with phone login, then restored as the launch channel — WhatsApp OTP needs a WABA + approved template, #42).
...
- ~~**CFDI Folio Pack Purchase** ([#24](https://github.com/jesushzv/business-helper/issues/24) / [#27](https://github.com/jesushzv/business-helper/issues/27))~~: **killed by the BYOK decision** (docs/STATUS.md §05) — both issues closed as not planned, the pack checkout code was removed in PR #225, and the folio ledger left the schema in `20260812182430` (#224). Reviving folio sales would need a new decision *and* a new migration.
...
- [x] **Quality Gate Compliance**: `npx vitest run` and `tsc --noEmit` pass. **Counts live in [`../STATUS.md`](../STATUS.md), not here** — this line has carried 138, 144, 175, 182 and 383, each stale within days. Lint is enforced at zero warnings: the script is `next lint --max-warnings=0` since [#46](https://github.com/jesushzv/business-helper/issues/46) closed.
- [x] **Playwright E2E Verification** ([#69](https://github.com/jesushzv/business-helper/issues/69)/[#91](https://github.com/jesushzv/business-helper/issues/91)): the suite was rewritten against the current flows, runs in CI (`e2e` job — note the mandatory `npx playwright install --with-deps chromium` step), and executes in the demo posture. Counts live in [`../STATUS.md`](../STATUS.md). The prior "14/14 passing" claim corresponded to no run.
...
- [ ] **Production API Keys Configuration**:
  ...
  - [ ] `OTP_DELIVERY_CHANNEL=email` + `RESEND_API_KEY` + `OTP_EMAIL_FROM` — **required, not optional.** (The Twilio/Meta variables this line used to demand are deprecated for OTP: email has been the launch channel since 2026-08-11 and the phone paths left the code in PR #315 — #319.) The earlier "Not Needed / Bypassed" note was wrong: `wa.me/` Click-to-Chat only covers *owner-initiated* messages. It cannot deliver an OTP to a signer, so without a provider the e-signature flow is inoperable and no quote can be signed. One Twilio account also covers the outbound reminders in `lib/whatsappOutbound.ts`. (Twilio SMS is the fastest to provision and needs no WABA; WhatsApp OTP requires the approved authentication template tracked in #42.)
  ...
  - [ ] ~~`FACTURAPI_SECRET_KEY`~~: **Removed** (BYOK, docs/STATUS.md §05, #221) — nothing reads it; do not set it. Every organization connects its own PAC in Ajustes. The Nota de Venta PDF and Accountant ZIP Export (`lib/receiptGenerator.ts`) remain the zero-SAT default.
