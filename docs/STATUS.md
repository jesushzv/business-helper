...
| 9 | **`CLAUDE.md` had 31 bytes of headroom, so adding a lesson forced re-flowing unrelated prose — and the cheap merge resolution then dropped other sessions' lessons under a green build** (mechanism in #135): the **Fabricated success** shape applied to the operating authority itself. P0 by decision, not by this section's money-and-compliance test. **PR #137 takes option B** — the catalogue moves to `docs/LESSONS.md`, `CLAUDE.md` drops to 15,736 bytes, and the two carry an explicit 32,000-byte *combined* ceiling (a decision, not a quiet bump) — and `tests/unit/lessonsCatalogue.test.ts` pins every issue number cited by a lesson in code, where a prose conflict cannot reach it, so a `--theirs` resolution goes red naming what vanished. **Remaining:** confirmation that B is the option wanted, and evidence the collision is gone. This file's own version — table rows, not headroom — is #138. | [#135](https://github.com/jesushzv/business-helper/issues/135) · [#138](https://github.com/jesushzv/business-helper/issues/138) |
