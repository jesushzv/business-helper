...
| **#2** — OTP provider configuration | Code merged; no credentials in the environment. `OTP_DELIVERY_CHANNEL` unset means 502 and **no quote can be signed**. Never tested on a real handset. WhatsApp is now the only channel — `sms` was retired with phone login (2026-08-10), so this is gated on a WhatsApp sender (Twilio sandbox/sender or Meta) plus the #42 template for cold recipients. | **Yes** — the core loop is dead without it |
...
| 2 | **Configure the WhatsApp OTP channel.** `sms` was retired 2026-08-10 (the product is email/OAuth + WhatsApp only), so the former "fastest to provision" path is gone: this now requires a WhatsApp sender — Twilio WhatsApp (sandbox works for a pilot whose signers join it) or the Meta Cloud API — and, for cold recipients, the approved authentication template tracked in #42. Set `OTP_DELIVERY_CHANNEL=whatsapp`, then verify a real code lands on a real handset and cannot be replayed. Per-recipient rate limiting is already in place (#20). Without this no quote can be signed at all, so it gates the end-to-end check for everything else. | [#2](https://github.com/jesushzv/business-helper/issues/2) · [#42](https://github.com/jesushzv/business-helper/issues/42) |
...
2. **Which WhatsApp provider — Twilio or Meta Cloud API?** Resolved to WhatsApp-only on 2026-08-10
   (`sms` retired with phone login); the provider choice within WhatsApp remains open. Is an account
   provisioned, and is WhatsApp Business API approval (which takes days) already in motion? #42's
   authentication template is now on the critical path either way.
