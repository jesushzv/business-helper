...
| Metric | State |
|:---|:---|
| Test suite | **2573 tests / 239 files**, `npx vitest run` (2026-08-17); typecheck, lint and `npm run build` clean in the same pass |
...
|:---|:---|:---|
| **Live PAC stamp** | The sandbox half is done and was worth doing: it found the integration entirely broken (`/v1` 410 since 2023, four payload defects that mocked-`fetch` coverage had kept green), and re-verified end to end afterwards — real SAT UUIDs, documents, cancellations, totals with and without retenciones. Forensics in the archive. **Re-scoped by BYOK (§05): the platform never stamps for a tenant**, so what remains is a tenant's own `sk_live_` key with CSDs connected in Ajustes, one stamp through `POST /api/invoices/issue`, and the UUID verifying at the SAT portal. A sandbox key cannot meet it — those documents have no fiscal validity and the route refuses them in production (`PAC_SANDBOX_KEY`). **This is a launch-day watch item, not a schedulable task**: no organization has a live PAC connected, so it is unreachable rather than refused. Preflight: `04-execution-testing/first-live-stamp-preflight.md`, which also carries the two unverified payload assumptions the first stamp settles — `factor: 'Exento'` (#407) and the complemento's zero-rate `TrasladoDR` (#408). | **Yes** — CFDI ships at launch |
...
- [ ] **A migration merged without being applied is caught at merge.** `npm run verify:migrations` compares `supabase/migrations/` to the target's ledger and exits non-zero when one is missing *or* when it could not check; the `Migrations applied to production` CI job runs it. **Unticked because the gate is inactive**: it needs `SUPABASE_DB_URL` as a repository secret, and says so loudly on every PR rather than passing quietly. Caveats and the hand-applied-ledger trap are in the script's own header
