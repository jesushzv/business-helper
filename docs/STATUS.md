...
| #88, #90 | Six 375px overflows closed; the header sticks below the demo banner via a measured `--bh-sticky-offset`; the cookie banner clears the bottom-pinned CTA |
| #99 | Convert-to-contract cannot double-fire (the route already 409s); CFDI stamping and PAC disconnect ask first, naming the folio cost; native dialogs gone. Cancel UI → #174 |
| #114, #124 | `isClientDemoMode()` stops honouring the sandbox flag once a session cookie exists; the dashboard treats an all-zero API answer as an answer, so a new tenant sees $0 |
