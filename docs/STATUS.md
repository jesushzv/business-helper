...
- [ ] **No migration in the repo is unapplied in production.** Vercel auto-deploys `main` and migrations are manual, so this row is the gap between the two (hard rule #6). Read it back from the live catalog, never from the ledger — the ledger does not list hand-applied work, which is why an earlier revision of this row was wrong in both directions. **Unticked by `20260816150000`** — see §02's blocking row for the read-back. Every earlier migration is applied and read back, the per-migration evidence in the archive. **This row is only ever as current as its last reconciliation — re-derive it, do not trust the tick.**
