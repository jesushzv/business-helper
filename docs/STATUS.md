...
| 9 | **`CLAUDE.md` had 31 bytes of headroom, so adding a lesson forced re-flowing unrelated prose — and the cheap merge resolution then dropped other sessions' lessons under a green build.** Three PRs open at once on 2026-08-09 (#125, #126, #120) rewrote the same paragraphs to buy the same few hundred bytes; #125 conflicted twice in an hour. The cost was never the merge: `git checkout --theirs CLAUDE.md` looks clean while discarding every lesson the other branch added, all tests green — the **Fabricated success** shape applied to the operating authority itself. P0 by decision, not by this section's money-and-compliance test. **PR #137 takes option B**: the catalogue moves to `docs/LESSONS.md`, `CLAUDE.md` drops to 15,736 bytes, and the two carry an explicit 32,000-byte *combined* ceiling — the ~6 KB rise over the old single budget is a decision, not a quiet bump. `tests/unit/lessonsCatalogue.test.ts` pins every issue number cited by a lesson in code, where a prose conflict cannot reach it, so a `--theirs` resolution goes red naming what vanished; each assertion was planted and watched fail. **Remaining:** confirmation that B is the option wanted, and evidence the collision is gone. This file's own version — table rows, not headroom — is #138. | [#135](https://github.com/jesushzv/business-helper/issues/135) · [#138](https://github.com/jesushzv/business-helper/issues/138) |
