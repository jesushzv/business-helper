...
> only (outbound WhatsApp messages stay; OTP delivery moved to email on 2026-08-11, with the
> phone channels deprecated). Its row dropped; eight remain.
...
| 2 | **Configure the email OTP channel.** Email via Resend became the launch channel on 2026-08-11 (§05 decision 2): one API key and a DNS-verified sending domain — no WABA, no carrier registration. Code and tests are in place (mocked `fetch` only); what remains is founder-side: create the Resend account, verify `businesshelper.app`, set `OTP_DELIVERY_CHANNEL=email` + `RESEND_API_KEY` + `OTP_EMAIL_FROM`, apply migration `20260811120000` (the ledger CHECK — without it every email send fails closed), run `npm run verify:otp` with `OTP_TEST_EMAIL`, then verify a real code lands in a real inbox and cannot be replayed. Signers now need `clients.email` on file — a client without one gets a 422 naming the field. Rate limiting is in place (#20). Gates the end-to-end check for everything else. | [#2](https://github.com/jesushzv/business-helper/issues/2) |
