...
| 5 | **Verify Stripe webhook signature enforcement** against a staging account. **Six of the eight checks pass against a real Next.js runtime** (2026-08-09); the two that remain — a signed event applied to a real row, and its redelivery refused — need a database, so they need a staging deployment. `npm run verify:webhook` exits non-zero and prints `INCOMPLETE` rather than reporting a pass without them. Detail in [`99-archive/status-log-2026-08.md`](99-archive/status-log-2026-08.md). Least blocking of the P0s — **but it decides whether a tenant who pays gets unblocked** once #128 ships. | [#63](https://github.com/jesushzv/business-helper/issues/63) |
...
- **`organizations.owner_id`: no index, and the *write* path assumed one org per owner**
  ...
  [#109](https://github.com/jesushzv/business-helper/issues/109)). Migration `20260811140000`
  **applied 2026-08-11**, read back: `idx_organizations_owner_id`, `indisunique false` (precondition
  checked first: 2 orgs, 0 owners holding two). **Option B** — multi-org stays possible, so not
  unique. #133 fixed the read path; this is the write half it did not touch: two owned rows broke
  PostgREST's singular-response check, so every settings and bank save answered 404 *"no se
  encontró una organización propia"*. `PATCH /api/organization` now filters by org `id` as well as
  `owner_id`; a member gets 403 naming who may write.
