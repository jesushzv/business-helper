...
| Metric | State |
|:---|:---|
| Test suite | **2533 tests / 238 files**, `npx vitest run` on the partial-payment branch cut from `main` (2026-08-16); typecheck, lint and build clean in the same pass |
...
|:---|:---|:---|
| **`record_milestone_payment`'s widened filter is not applied** | `20260816150000` lets a `marked_paid` cobro with a balance stay payable (#382/#371); the two it sits on are applied and proven (§04). Until it lands, a client returning to pay a remainder still gets 409 — nothing is fabricated, since the widening only *admits* rows the old filter refused. `db:migrate:dry`, apply, then read back: `pg_get_functiondef` must show `status IN ('pending', 'requested', 'marked_paid')`, `prosecdef = false`, EXECUTE only `postgres`/`service_role`. **Unchecked from this session** — the connector answered `MCP error -32003: requires approval` to `list_projects` twice; re-test rather than inherit that (#129). | **No** — the pay step works; paying a remainder does not |
...
- [ ] **No migration in the repo is unapplied in production.** Vercel auto-deploys `main` and migrations are manual, so this row is the gap between the two (hard rule #6). Read it back from the live catalog, never from the ledger — the ledger does not list hand-applied work, which is why an earlier revision of this row was wrong in both directions. **Unticked again by `20260816150000` (`record_milestone_payment`'s widened status filter, #382/#371) — merged, not applied**; the read-back it needs is in §02's blocking row. The four before it are applied and read back:
  - `20260815200000` (`record_owner_payment`, [#394](https://github.com/jesushzv/business-helper/issues/394)) — applied 2026-08-16 and read back: `prosecdef = false`, EXECUTE only `postgres`/`service_role`, both guards proven by rejection (zero amount → `22023`, unknown milestone → NULL, ledger row count unchanged). It reached production unapplied first and took the confirm step down for a day; the account is in the archive.
  - `20260814080000` (`clients.archived_at`, [#337](https://github.com/jesushzv/business-helper/issues/337)) — column and `idx_clients_org_active` read back present. The migration's own header still says "NOT yet applied"; that comment is stale, not the schema.
