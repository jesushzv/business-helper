...
| `OTP_DELIVERY_CHANNEL` | OTP channel selector | 🔴 **Unset — flow inoperable** | **Launch Gate — P0** → [#2](https://github.com/jesushzv/business-helper/issues/2). Must be `whatsapp` (`sms` was retired with phone login and fails closed); unset means `POST /api/quotes/public/[token]/otp` returns 502 and no quote can be signed. |
...
| `TWILIO_WHATSAPP_NUMBER` | Twilio WhatsApp sender | 🔴 Unset | **Launch Gate — P0** → [#2](https://github.com/jesushzv/business-helper/issues/2). (`TWILIO_SMS_NUMBER` belonged to the retired sms channel and is no longer read.) |
