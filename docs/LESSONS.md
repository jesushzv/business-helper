...
- **Hooks and mutations** (#33, #50, #59): every mutation applies the **server row** on success and
  throws/surfaces on failure — never an optimistic local object. Demo fixtures only behind
  `isClientDemoMode()`. Pin with an `*Honesty.test.ts` (`useReceivablesHonesty.test.ts` is the
  template). **Fixing the hook does not fix the form above it** (#95): `useState(prop)` that never
  re-syncs keeps the typed text after the server row lands, and the server normalizes
  (`'81 1234 5678'` → `'8112345678'`) — a banner beside a value the DB does not hold. Re-sync on the
  prop (`OrgProfileCard`).
- **Public pages may simulate a mutation only behind `isClientDemoMode()`, short-circuited *before*
  the fetch — never as a `catch` fallback** (#58, #86), which turns a real tenant's network failure
  into a fake confirmation: `/pay/[token]` told a payer "Comprobante enviado correctamente" for a
  declaration the API had rejected.
- **Placeholder identifiers are the same rule in a UI costume** (#44, #78, #96): `token || 'demo'`,
  `regimen_fiscal || '601'` render as a live control or a settled fact. Absent is absent — render the
  **disabled** control and **name the record** to fix; a missing CLABE reported as a missing phone
...
- **Demo-mode detection differs by side.** Collection GET routes answer the demo deployment with
  ...
  the build-time signal `isClientDemoMode()` (`lib/clientDemoMode.ts`). Wrong either way: a blank
...
- **A hook gating a warning or a disabled control needs three states**: `true` / `false` /
  **`null` = unknown** (loading, or the read failed). Collapsing unknown into `false` invents a fact
  — warning a healthy tenant on a network blip; into `true` re-opens the hole the gate closed. Never
  let the client be the only enforcement: gate on the server too and the permissive choice for
  unknown becomes safe (`lib/hooks/useSettlementAccount.ts`, #64). **Any figure derived from a
  fetched list needs the same** — a client page rendered "Disponible $50,000" from a failed read
  (#96).
...
- **Never edit a migration after it has been applied anywhere.** `ADD COLUMN IF NOT EXISTS` is
  ...
  leaves repo and production silently disagreeing. Reconcile the live DB explicitly (`ALTER` through
  the connector, read back) or add a new migration, saying which in the PR. New money columns are
  `numeric(12,2)`; new CHECKs take the `chk_` prefix.
...
- **A vocabulary the database enforces exists once in code, with `null` for everything else**
  ...
  event was claimed, and
  the display read the unknown word as "Cancelado" to a customer who had just paid. Export the union
  and a `normalize*` returning `null`; write only what the event established. It survived because
  the tests fed one object shape — feed the others the event can carry.
...
- **A form the user cannot get past is usually validation that returns at the first failure and
  ...
  and 400'd at the first, so each submit revealed one more problem, in one banner atop a modal
  taller than a 375px viewport — tapping *Guardar* looked like nothing happened. Validate
...
- **localStorage is demo-sandbox state, never a real tenant's store.** Real tenants read the API
  ...
  or throw (#93/#96; same class as #33/#50/#58). **The bare `catch` around the fetch is where it
  hides**: `useReceivables` fell through to fixtures on any network error, so one dropped request
  filled Cobranza with ~$145,000 owed by companies that do not exist, `error` left null (#96). Copy
  `useClients` — `if (!isClientDemoMode()) { …fetch, set error, return }` before any localStorage
  path — and audit all four surfaces: read, write mirror, `resetDemo*`, mutations. Writes too: a
  local "confirmed" keyed off `BACKEND_NOT_CONFIGURED` had a production reporting payments nobody
  received.
- **A catalogue is `lib/`, never a `const` in a screen** (#127) —
  `tests/unit/satRegimenCatalogue.test.ts` is the gate. A **stored value the list omits is not
  absent**: give it its own option (`621 — (código guardado)`), never the blank `<select>` that
  showed a tenant no régimen while the database held one.
...
- CI has been **silently absent** on a draft PR for ten hours while Vercel showed green (#38), and
  skips the odd later push. Compare the run's head SHA to the PR's: absence looks like passing, and
  a merge commit re-triggers it.
- E2E is not in CI: never cite Playwright results you did not run.
...
- **In a remote session, local `main` can be stale — cut branches from `origin/main` after an
  ...
  *before the issue tracker existed*, so a branch cut from it rebuilds weeks-old code.
...
- **A session with the Supabase connector can run "needs a deployment" checks itself — don't park
  ...
  issue**: #96 was merged with just its deployed check outstanding, and running it found a table
  missing three columns the shipped code read. Only the browser session and the real credential are
  out of reach; schema, grants, constraints, PostgREST **and the deployed app itself** are not — an
  `@supabase/ssr` cookie on `extensions.http(('PATCH','https://…'))` reaches it (recipe in #129),
  and the shell's `403` on the app domain is not the last word (#95 sat three weeks on that).
  **Nor is GoTrue** — `/auth/v1/settings?apikey=<anon>` returns the `external` provider map,
  `/auth/v1/authorize?provider=…` the user's error, `auth.identities` who has *ever* signed in that
  way; #48 sat two days on it.
