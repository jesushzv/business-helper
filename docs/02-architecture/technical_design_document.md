...
|:---|:---|:---|:---|
| **Unit Tests** | Node.js Native Runner (`node --test`) | Tax withholding functions, RFC Modulo 11 parser, SHA-256 seals. Min 85% coverage. | Husky pre-commit & PR |
| **Integration Tests** | Playwright API Client | API route responses, error codes, RLS policy bypass attempts. | PR merge check |
| **E2E Tests** | Playwright Chrome/Webkit | Happy path: Quote creation → WhatsApp link → Client OTP sign → Payment confirm. | Production Deploy |
